        ...
        for(int i = 0; i < observations.length; ++i)
        {
            ...
            for(int j = 0; /*!reached &&*/ j < observations[i].size(); ++j)
            {
                ...
                if(pathToObservation != null && pathToObservation.size() > 0)
                {
                    ...
                    /*double blocksStraightLine = avatarPos.dist(obs.position)/block_size;
                    int blocksAStar = pathToObservation.size();

                    //System.out.println("BlockAStar: " + blocksAStar + ", blocksStraight: " + blocksStraightLine);
                    if(blocksAStar > blocksStraightLine)
                    {
                    }     */
    ...
    private boolean followPath(StateObservation stateObs, ArrayList<Node> toFollow, int block_size, int targetID)
    {
        ...
        for(int i = 0; i < numNodes; ++i)
        {
            ...
            if(!success)
            {
                ...
                {
                    TreeSet<core.game.Event> events = lastAdvancedState.getEventsHistory();
                    for(Event e : events)
                    {
